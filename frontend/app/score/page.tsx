...
type Step = "idle" | "fetching" | "review" | "paying" | "proving" | "submitting" | "done" | "error";
  ...
  async function handleMintCredential(proofResult: ProofResult) {
    ...
    try {
      writeContract({
        ...
        args: [
          proofResult.proof_bytes as `0x${string}`,
          proofResult.instances_uint256.map(BigInt),
        ],
      });
    } catch {
      // Credential minting failed (e.g., no contract deployed yet) — still show proof success
      setStep("done");
    }
  }

  const tierConfig = proof?.tier !== undefined ? getTierConfig(proof.tier) : null;
  ...
  return (
    <div className="min-h-screen gradient-bg">
      ...
      <main className="max-w-2xl mx-auto px-6 py-12">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">ZK Credit Score</h1>
        ...
        {step === "fetching" && (
          <div className="glass rounded-2xl p-10 text-center">
            ...
            <p className="text-white/60">Fetching on-chain signals for {address?.slice(0, 8)}...</p>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="glass rounded-2xl p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-semibold text-white">Your On-Chain Signals</h2>
                <div className="px-3 py-1 rounded-full bg-white/5 text-xs text-white/40 font-mono">
              ...
              <div className="bg-violet-500/10 border border-violet-500/20 rounded-xl p-4 mb-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-3xl font-bold text-white">{signals.estimated_score}</div>
                    <div className="text-sm text-violet-300 mt-1">→ Tier {signals.estimated_tier_label}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-white/30 mb-1">Collateral Ratio</div>
                    <div className="text-2xl font-bold text-white">{[150, 120, 90, 70][signals.estimated_tier]}%</div>
                    <div className="text-xs text-white/30 mt-1">vs 150% standard</div>
                  </div>
                </div>
              </div>

              <div className="text-xs text-white/30 mb-4 p-3 bg-white/3 rounded-lg">
              ...
              <button
                ...
                className="w-full py-4 bg-violet-600 hover:bg-violet-500 text-white font-bold rounded-xl transition-all glow-purple text-lg"
              >
                Generate ZK Proof →
              </button>
            </div>
        ...
        {step === "proving" && (
          <div className="glass rounded-2xl p-10">
            <h2 className="text-xl font-bold mb-6 text-center">Generating ZK Proof</h2>
            ...
            <div className="space-y-4 mb-8">
              {[
                ...
                <div key={item.label} className="flex items-center gap-3">
                  ...
                  <span className={`text-sm transition-colors ${item.done ? "text-white" : "text-white/30"}`}>
                    {item.label}
                  </span>
                </div>
              ))}
            ...
            <div className="text-center text-sm text-white/40 font-mono">{Math.round(proofProgress)}%</div>
          </div>
        )}

        {/* Step: Submitting tx */}
        {(step === "submitting" || txLoading) && proof && (
          <div className="glass rounded-2xl p-10 text-center">
            <div className="w-12 h-12 border-2 border-violet-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" />
            <h2 className="text-xl font-bold mb-2">Submitting to HashKey Chain</h2>
          ...
          <div className="space-y-4">
            ...
            <div className={`rounded-2xl p-8 border-2 ${
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="text-center">
                  <div className="text-3xl font-bold text-white">{proof.score}</div>
                  <div className="text-xs text-white/40 mt-1">ZK Score</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl font-bold text-white">{proof.collateral_ratio}%</div>
                  <div className="text-xs text-white/40 mt-1">Collateral Ratio</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl font-bold text-white">{proof.generation_time_ms}ms</div>
                  <div className="text-xs text-white/40 mt-1">Proof Time</div>
                </div>
              </div>
              ...
              <div className="bg-black/40 rounded-xl p-4 font-mono text-xs space-y-2">
                <div className="flex justify-between">
                  <span className="text-white/30">Proof Mode</span>
                  <span className="text-violet-300">{proof.proof_mode?.toUpperCase()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/30">Proof Hash</span>
                  <span className="text-white/60">{proof.proof?.slice(0, 20)}...</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/30">Public Instance</span>
                  <span className="text-white/60">{proof.instances_uint256?.[0]?.slice(0, 12)}...</span>
                </div>
                {txHash && (
                  <div className="flex justify-between">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Link
                href="/lend"
                className="py-4 text-center glass hover:bg-white/10 text-white font-semibold rounded-xl transition-all"
              >
                Use in Lending →
              </Link>
              <Link
                href="/pledge"
                className="py-4 text-center bg-violet-600/30 border border-violet-500/30 hover:bg-violet-600/50 text-violet-300 font-semibold rounded-xl transition-all"
              >
                Create Pledge →
              </Link>
            </div>
        ...
        {step === "error" && (
          <div className="glass rounded-2xl p-8 border border-red-500/20">
            ...
            <p className="text-white/60 text-sm mb-4 font-mono">{error}</p>
            <button
